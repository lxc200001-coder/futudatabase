...
def _build_slice_rows(df, mask, ma_len, ktype, ha_ma_val, direction, signal,
    ...
    for j in range(n_sl):
        ...
        rows.append({
            ...
            "close_amount": float(_vp_amt[j]) if _vp_amt[j] is not None else None,
            ...
            "close_type": str(trade_status_arr[j]) if trade_status_arr[j] == "已平仓" else ("虚拟平仓" if trade_status_arr[j] == "持仓中" else None),
...
def run_backtest(ktype="1w", ma_list=None, ma_start=2, ma_end=61, ma_step=1,
    ...
    _cols = ["code","stock_name","market","ktype","window_label","datetime",
        ...
        "close_shares","close_slippage",
        "close_amount","close_commission","close_pnl","close_type",
