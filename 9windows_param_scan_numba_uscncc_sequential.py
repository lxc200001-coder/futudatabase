...
PCT_COLS = [
    ...
    "平均每笔收益率", "收益率", "年化收益率", "买入持有收益率", "超额收益率",
    "最大回撤", "盈利交易率",
]
...
@njit
def _numba_sequential_backtest(close, buy, sell, initial_cash, fee_rate,
                                init_shares=0, init_entry_price=0.0, init_entry_idx=-1,
                                force_close_end=False):
    """Numba 加速连续回测：支持初始持仓。

    返回 (trades_arr, equity_arr, n_trades, final_cash, final_shares, final_entry_price, final_entry_idx)。
    """
    n = len(close)
    max_trades = n
    trades = np.zeros((max_trades, 13))
    ...
    # 末尾强平（仅当 force_close_end 为 True 且过渡期结束时）
    ...
    final_entry_price = entry_price_val
    final_entry_idx = entry_idx

    return trades[:trade_count], equity, trade_count, final_cash, final_shares, final_entry_price, final_entry_idx
...
def _run_sequential(code, df, windows, stock_name, stock_plates, stock_all_rows=None):
    ...
    后续窗口为测试集（每窗口用选出的最优 MA 交易，持仓连续跨窗口）。
    """
    ...
    equity_curves = []     # 各窗口资金曲线片段
    ma_history = []        # 每窗口使用的 MA
    signal_map = {}        # 信号扫描用
    ...
    if current_ma is None:
        return [], pd.DataFrame(), {}, None  # 无可用 MA，跳过
    ...
    for ws, we in test_windows:
        ...
        # 计算新旧 MA 信号
        df_w["ma_old"] = ma_cache[current_ma][mask]
        df_w["dir_old"] = np.where(df_w["ma_old"] > df_w["ma_old"].shift(1), 1, -1)
        df_w["buy_old"] = (df_w["dir_old"] == 1) & (df_w["dir_old"].shift(1) == -1)
        df_w["sell_old"] = (df_w["dir_old"] == -1) & (df_w["dir_old"].shift(1) == 1)

        if _next_ma is not None:
            df_w["ma_new"] = ma_cache[_next_ma][mask]
            df_w["dir_new"] = np.where(df_w["ma_new"] > df_w["ma_new"].shift(1), 1, -1)
            df_w["buy_new"] = (df_w["dir_new"] == 1) & (df_w["dir_new"].shift(1) == -1)
            df_w["sell_new"] = (df_w["dir_new"] == -1) & (df_w["dir_new"].shift(1) == 1)

        if in_transition:
            # 过渡期：用旧 MA 信号平仓，平仓后切新 MA
            sell_signal = df_w["sell_old"].values.copy().astype(np.bool_)
            buy_signal = np.zeros(len(df_w), dtype=np.bool_)
            ...
            if transition_idx >= 0:
                # 过渡期后：用新 MA 信号
                for i in range(transition_idx, len(df_w)):
                    if _next_ma is not None:
                        buy_signal[i] = df_w["buy_new"].iloc[i]
                        sell_signal[i] = df_w["sell_new"].iloc[i]
                # 切换完成
                current_ma = _next_ma
            else:
                # 未平仓，结束时强平（force_close_end 处理）
                # 仍在过渡期，无新买入
                pass
        else:
            # 正常：用当前 MA 信号
            current_ma = _next_ma if _next_ma is not None else current_ma
            df_w["ma"] = ma_cache[current_ma][mask]
            df_w["dir"] = np.where(df_w["ma"] > df_w["ma"].shift(1), 1, -1)
            buy_signal = ((df_w["dir"] == 1) & (df_w["dir"].shift(1) == -1)).values.astype(np.bool_)
            sell_signal = ((df_w["dir"] == -1) & (df_w["dir"].shift(1) == 1)).values.astype(np.bool_)

        # ---- 执行回测 ----
        close_arr = df_w["close"].values.astype(np.float64)
        force_close = in_transition and transition_idx < 0  # 过渡期未平才强平

        trades_arr, equity_arr, n_trades, final_cash, final_shares, \
            final_entry_price, final_entry_idx = _numba_sequential_backtest(
                close_arr, buy_signal, sell_signal, cash, FEE_RATE,
                position_shares, position_price, position_idx,
                force_close
            )
        ...
        if n_trades == 0 and position_shares == 0:
            ...
            summary = {
                "股票代码": code, "市场": "",
                "K线周期": BAR_INTERVAL, "均线周期": current_ma,
                "收益率": 0, "年化收益率": 0,
                "买入持有收益率": 0, "超额收益率": 0,
                "最大回撤": 0, "夏普比率": 0, "卡尔玛比率": 0,
                "交易次数": 0, "盈利交易率": 0, "盈利因子": 0, "盈亏比": 0,
                "平均盈利": 0, "平均亏损": 0, "最大单笔盈利": 0, "最大单笔亏损": 0,
                "最大连续盈利次数": 0, "最大连续亏损次数": 0, "平均持仓天数": 0,
                "初始资金": cash, "最终资金": cash,
            }
        else:
            trades_df_for_summary = _trades_arr_to_df(trades_arr, n_trades, df_w, code, stock_name, stock_plates) if n_trades > 0 else pd.DataFrame()
            summary = build_summary(trades_df_for_summary, current_ma, df_w, equity_arr=equity_arr)
        ...
        position_price = final_entry_price
        position_idx = final_entry_idx

        ma_history.append(current_ma)

        equity_curves.append(equity_arr)
    ...
    # 收集信号扫描数据（用最新 MA）
    last_ma = ma_history[-1] if ma_history else current_ma
    # 获取信号
    _sig_df = df.tail(len(df)).copy()
    _sig_df["ha_close"] = ha_close_full.tail(len(df))
    _sig_df["ma"] = ma_cache[last_ma].tail(len(df)) if last_ma in ma_cache else pd.Series(dtype=float)
    _sig_df["dir"] = np.where(_sig_df["ma"] > _sig_df["ma"].shift(1), 1, -1)
    _sig_df["buy"] = (_sig_df["dir"] == 1) & (_sig_df["dir"].shift(1) == -1)
    _sig_df["sell"] = (_sig_df["dir"] == -1) & (_sig_df["dir"].shift(1) == 1)
    ...
    signal_info["是否全市场成交额前200"] = "是" if code in _load_top_turnover_set() else "否"
    signal_info["K线周期"] = BAR_INTERVAL
    signal_info["均线周期"] = last_ma
    signal_info["市场"] = str(df["market"].iloc[0]) if "market" in df.columns else ""
    signal_map[code] = {last_ma: signal_info}

    # 合并交易记录
    all_trades_merged = pd.concat(all_trades, ignore_index=True, sort=False) if all_trades else pd.DataFrame()

    return all_summary_rows, all_trades_merged, signal_map, last_ma


def _trades_arr_to_df(trades_arr, n_trades, df_w, code, stock_name, stock_plates):
    """将 numba 输出交易数组转为 DataFrame。"""
    ...
    for j in range(n_trades):
        ...
        records.append({
            ...
            "均线周期": 0,  # 在主循环中会覆盖
            "开仓时间": entry_time,
            "开仓价格": entry_price_val,
            "买入股数": position_val,
            "买入手续费": round(buy_fee, 2),
            "平仓时间": exit_time,
            "平仓价格": t[3],
            "卖出手续费": round(sell_fee, 2),
            "总手续费": round(total_fee, 2),
            "收益金额": round(pnl, 2),
            "盈亏百分比": round(return_pct, 2),
            "持仓K线数": int(t[4]) - int(t[2]),
            "持仓天数": hold_days,
            "平仓类型": "强平" if is_force else "正常",
        })
...
def _process_one_stock(code, windows=None, mode="window"):
    ...
    # 信号扫描用连续模式的最新 MA
    _seq_sig_map = {}
    if _sig_outer and code in _sig_outer:
        _seq_sig_map = _sig_outer[code]

    # stability 用连续模式的 summary
    _seq_ws = None
...
def _write_summary_excel(out_path, signal_df, all_df, score_matrix, rank_matrix,
    ...
    with pd.ExcelWriter(out_path, engine="openpyxl") as writer:
        ...
        logic_rows = [
            ...
            {"类型": "回测指标", "名称": "平均每笔收益率",
             "统计逻辑": "所有交易盈亏百分比的算术平均值 = sum(每笔盈亏百分比) / 交易次数，衡量单笔交易的平均收益水平"},
            {"类型": "回测指标", "名称": "最大回撤",
            ...
            {"类型": "Walk-Forward", "名称": "训练窗口选出的最优MA",
             "统计逻辑": "从当前训练集的参数稳定性分析中选出的最优均线周期，用于当前测试窗口交易"},
            {"类型": "Walk-Forward", "名称": "当前测试窗口所用均线周期是否与上个测试窗口所用均线周期一致",
             "统计逻辑": "是=与上个测试窗口MA相同（无切换），否=MA发生变化（可能进入过渡期）"},
            {"类型": "Walk-Forward", "名称": "当前测试窗口是否存在上个测试窗口的未平仓交易",
             "统计逻辑": "是=窗口开始时有持仓延续自上个窗口，否=窗口开始时无持仓"},
            {"类型": "Walk-Forward", "名称": "过渡期采用的最优均线周期",
             "统计逻辑": "过渡期使用的旧MA（当前测试窗口MA变化时，旧仓位仍用旧MA信号平仓），无过渡为0"},
            # ── 参数选择 ──
            {"类型": "参数选择", "名称": "最优均线周期",
