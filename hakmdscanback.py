...
def build_trades(df, ma_len):
    ...
    for i in range(len(df)):
        ...
        # ================= SELL =================
            ...
            trades.append({
                ...
                "开仓前可用现金": round(pre_cash, 2),
                "开仓后可用现金": round(pre_cash - cost - buy_fee, 2),
                "平仓后可用现金": round(post_cash, 2),
    ...
    if position > 0:
        ...
        trades.append({
            ...
            "开仓前可用现金": round(pre_cash, 2),
            "开仓后可用现金": round(pre_cash - entry_price * position - buy_fee, 2),
            "平仓后可用现金": round(post_cash, 2),

            "持仓K线数": len(df) - entry_index,
            "持仓天数": (len(df) - entry_index) * 7
        })
