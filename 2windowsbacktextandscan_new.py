        ...
        with pd.ExcelWriter(out, engine="openpyxl") as writer:
            ...
            if window_rows and stability_dfs:
                ...
                if not best_score_strategy.empty:
                    ...
                    compare["策略表现"] = compare["最终选择均线周期回测结果综合评分"].apply(_stock_nature)

                    # 排序：参数一致性检验升序 → 策略表现升序 → 综合评分降序
                    compare["_sort_key"] = compare["参数一致性检验"].map({"一致": 0, "不一致": 1})
                    compare = compare.sort_values(
                        ["_sort_key", "策略表现", "最终选择均线周期回测结果综合评分"],
            ...
            if compare is not None and not compare.empty:
                selected_map = compare[["股票代码", "最终选择均线周期", "策略表现"]].rename(
            ...
            logic_rows = [
                ...
                {"类型": "基础信息", "名称": "股票名称",
                 "统计逻辑": "从 data/stocks_plates.parquet 关联，源自富途 get_stock_basicinfo"},
                {"类型": "基础信息", "名称": "所属板块",
                 "统计逻辑": "从 data/stocks_plates.parquet 关联，由 get_owner_plate 获取后聚合（排除OTHER类型），逗号拼接"},
                {"类型": "基础信息", "名称": "策略表现",
                 "统计逻辑": "根据最终选择均线周期回测结果综合评分分档：>=80为1优，>=60为2良，>=40为3中，>=20为4差，<20为5劣"},
                {"类型": "基础信息", "名称": "时间",
                 "统计逻辑": "最新一根K线的时间"},
                {"类型": "基础信息", "名称": "收盘价",
                 "统计逻辑": "最新一根K线的收盘价"},
                {"类型": "基础信息", "名称": "HA收盘价",
                 "统计逻辑": "最新一根K线的Heikin Ashi收盘价，(open+high+low+close)/4"},
                {"类型": "基础信息", "名称": "HA均线值",
                ...
                {"类型": "参数选择", "名称": "全量回测最优均线周期",
                 "统计逻辑": "全量回测中综合评分最高的均线周期"},
                {"类型": "参数选择", "名称": "全量回测最优均线周期回测结果综合评分",
                 "统计逻辑": "全量回测最优均线周期对应的综合评分"},
                {"类型": "参数选择", "名称": "全量回测最优均线周期参数稳定性综合评分",
                 "统计逻辑": "全量回测最优均线周期在窗口回测中的参数稳定性综合评分"},
                {"类型": "参数选择", "名称": "窗口回测最优均线周期",
                 "统计逻辑": "窗口回测参数稳定性分析中综合评分最高的均线周期"},
                {"类型": "参数选择", "名称": "窗口回测最优均线周期回测结果综合评分",
                 "统计逻辑": "窗口回测最优均线周期在全量回测中的综合评分"},
                {"类型": "参数选择", "名称": "窗口回测最优均线周期参数稳定性综合评分",
                 "统计逻辑": "窗口回测最优均线周期对应的参数稳定性综合评分"},
                {"类型": "参数选择", "名称": "参数一致性检验",
                 "统计逻辑": "全量最优与窗口最优的均线周期是否一致"},
                {"类型": "参数选择", "名称": "综合评分差值比",
                 "统计逻辑": "|全量最优综合评分 - 窗口最优综合评分| / max(两者综合评分)，衡量参数差异程度"},
                {"类型": "参数选择", "名称": "参数稳定性综合评分差值比",
                 "统计逻辑": "|全量最优稳定性评分 - 窗口最优稳定性评分| / max(两者稳定性评分)"},
                {"类型": "参数选择", "名称": "全量参数两项评分加权得分",
                 "统计逻辑": "全量最优均线周期的综合评分×0.6 + 参数稳定性综合评分×0.4"},
                {"类型": "参数选择", "名称": "窗口参数两项评分加权得分",
                 "统计逻辑": "窗口最优均线周期的综合评分×0.6 + 参数稳定性综合评分×0.4"},
                {"类型": "参数选择", "名称": "最终选择均线周期来源",
                 "统计逻辑": "加权得分高的一方：全量或窗口"},
                {"类型": "参数选择", "名称": "最终选择均线周期",
                 "统计逻辑": "最终选定使用的均线周期，用于信号扫描筛选"},
                {"类型": "参数选择", "名称": "最终选择均线周期回测结果综合评分",
                 "统计逻辑": "最终选择均线周期在全量回测中的综合评分"},
                ...
                {"类型": "参数稳定性", "名称": "综合评分排名平均值",
                 "统计逻辑": "该均线周期在各窗口中综合评分排名的算术平均值"},
                {"类型": "参数稳定性", "名称": "综合评分排名第一次数",
                 "统计逻辑": "该均线周期在各窗口中排名第一的次数"},
            ]
