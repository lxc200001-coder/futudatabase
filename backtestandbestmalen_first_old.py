    ...
    for i in range(len(df)):
        ...
        # =========================
            ...
            trades.append({
                ...
                "持仓K线数": i - entry_index,
                "持仓天数": (pd.to_datetime(time) - pd.to_datetime(entry_time)).days,
...
def build_summary(trades_df, ma_len, df):
    ...
    return {
        "股票代码": trades_df["股票代码"].iloc[0],
        "均线周期": ma_len,
        "收益率": round(total_return_pct, 2),
        "年化收益率": round(annual_return, 2),
        "交易次数": total_trades,
        "盈利交易率": round(win_rate, 2),
        "盈利因子": round(profit_factor, 2) if np.isfinite(profit_factor) else "inf",
        "回测周期": backtest_period
    }
...
def run_trade():
    ...
    if all_summary_rows:
        ...
        rule_df = pd.DataFrame([
            {
                "Sheet名称": "全部回测",
                "筛选逻辑": "所有股票 + 所有均线参数完整回测结果"
            },
            {
                "Sheet名称": "最佳参数",
                "筛选逻辑": "每只股票取年化收益率最高的一组均线参数"
            },
            {
                "Sheet名称": "优秀股票",
                "筛选逻辑": "年化收益率>=20 且 交易次数>20 且 盈利交易率>45"
            }
        ])

        with pd.ExcelWriter(
                os.path.join(TRADE_DIR, "all_summary.xlsx"),
                engine="openpyxl"
        ) as writer:
            all_df.to_excel(writer, sheet_name="全部回测", index=False)

            best_df.to_excel(writer, sheet_name="最佳参数", index=False)

            excellent_df.to_excel(writer, sheet_name="优秀股票", index=False)

            rule_df.to_excel(writer, sheet_name="筛选规则", index=False)
    ...
    if failed_symbols:
        ...
        failed_df.to_excel(
            os.path.join(TRADE_DIR, "failed_symbols.xlsx"),
            index=False
        )
